...
(`lib/cumba-oss-datatable`, `lib/cumba-oss-datatable-impl`, etc.). Children inherit:
...
> **Removed: `dev` and `main`.** This pom used to declare an
> `activeByDefault` `dev` profile and a `-P main` release profile, each
> carrying an *identical* `<modules>` list. That did not work. Maven
> deactivates every `activeByDefault` profile as soon as any other profile
> activates, and `PMD` / `SpotBugs` / `NullAway` below self-activate
> unconditionally via `!property` rules — so `dev` was never active.
> Measured: `mvn validate` with no `-P` built the parent pom **alone**
> (1 module) while `-P main` built 13. CI passed `-P main` on every
> invocation, so it exercised a configuration no developer ever ran.
>
> `<modules>` now sits at **top level**, where profile activation cannot
> reach it, and the strict `-Xlint:all` that `dev` carried (and which had
> therefore never actually run — verified clean before adopting it) is now
> unconditional in `maven-compiler-plugin`'s configuration. The two
> coverage sub-modules the profiles selected between collapsed into a
> single `coverage/`.
...
- **In-house module declarations** (`cumba-oss-datatable`, `cumba-oss-datatable-impl`,
  `cumba-oss-datatable-provider-csv`, …) so children can pull each other in without
  versioning.
